...
def train(args):
    ...
    if isres:
        models_dir = os.path.join(workspace, 'att_res_models', subdir)
    else:
    ...
    for (iteration, (batch_x, batch_y, _)) in enumerate(generator.generate_train()):
        ...
        if iteration % 100 == 0:
            ...
            logging.info('se: {:.4f}, sp: {:.4f}, as: {:.4f}, hs: {:.4f}, tr_loss: {:.4f}'.format(se, sp, as_score, hs_score, tr_loss))
            ...
            logging.info('se: {:.4f}, sp: {:.4f}, as: {:.4f}, hs: {:.4f}, va_loss: {:.4f}'.format(se, sp, as_score, hs_score, va_loss))
...
def inference_validation_data(args):
    ...
    if isres:
        model_path = os.path.join(workspace, 'att_res_models', subdir, 'md_{}_iters.tar'.format(iteration_max))
    else:
